...
(async () => {
  ...
  async function mergeToYamatoTemplate(csvFile, templateUrl, sender) {
    ...
    rows.slice(start).forEach(r => {
      sheet[`A${rowExcel}`] = { v: r[1] || "" };   // お客様管理番号
      sheet[`B${rowExcel}`] = { v: "0" };          // 送り状種類
      sheet[`C${rowExcel}`] = { v: "0" };          // クール区分
      sheet[`E${rowExcel}`] = { v: shipDate };     // 出荷予定日
      sheet[`I${rowExcel}`] = { v: r[13] || "" };  // 電話番号
      sheet[`K${rowExcel}`] = { v: r[10] || "" };  // 郵便番号
      sheet[`L${rowExcel}`] = { v: r[11] || "" };  // 住所1
      sheet[`M${rowExcel}`] = { v: r[12] || "" };  // 氏名
      sheet[`U${rowExcel}`] = { v: "ブーケフレーム加工品" }; // 品名1
      sheet[`BF${rowExcel}`] = { v: sender.name };
      sheet[`BG${rowExcel}`] = { v: sender.phone };
      sheet[`BI${rowExcel}`] = { v: sender.postal };
      sheet[`BJ${rowExcel}`] = { v: sender.address };
      rowExcel++;
    });
